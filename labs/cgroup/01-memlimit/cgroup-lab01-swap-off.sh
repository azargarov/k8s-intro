...
echo "setting memory.max to 50 MiB ( 50 bytes)"
echo " 50 " | sudo tee "/sys/fs/cgroup/demo-mem/memory.max" >/dev/null
echo "memory.max: $(cat "/sys/fs/cgroup/demo-mem/memory.max")"

echo "setting memory.swap.max to 0"
echo 0 | sudo tee "/sys/fs/cgroup/demo-mem/memory.swap.max" >/dev/null

echo "memory.swap.max: $(cat "/sys/fs/cgroup/demo-mem/memory.swap.max")"
